...
- A **sync watchdog** runs when you open the app and whenever a Bluetooth event arrives: if nothing has reached Drive successfully for over 6 hours — counted from the later of the last success and the moment you signed in — it cancels and re-registers the hourly job, which is what shakes loose a periodic worker the OS has quietly stopped dispatching. For as long as those 6 hours stand, the main screen carries a **"Sync may be stalled"** banner with a one-tap **Sync now**. The forced re-registration then gets an hour to take effect; if it hasn't by then, a **"Bluetooth Logger has stopped syncing"** notification follows and the app keeps forcing. The next successful sync clears the notification, and so does signing out.
...
- `POST_NOTIFICATIONS` — the setup-health warning and the "has stopped syncing" stall alert.
