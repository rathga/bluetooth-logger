...
- Idempotency: per-month last-synced byte offset persisted in `SharedPreferences`. Manual "Sync now" button in the UI enqueues a one-off run of the worker for debugging; if a sync is already in flight it records the skip in the on-device journal without touching Drive and asks WorkManager to retry it, so an overlapped run is deferred rather than dropped.
