...
- Idempotency: per-month last-synced byte offset persisted in `SharedPreferences`. Manual "Sync now" button in the UI enqueues a one-off run of the worker for debugging; it returns without touching Drive if a sync is already in flight.
