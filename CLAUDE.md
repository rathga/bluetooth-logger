...
- `setup/SetupNotifier.kt` — channel creation + post/cancel notifications (setup-health warning; `notifyAuthNeeded`/`clearAuthNeeded` for a re-sign-in prompt on auth failure; `notifySyncStalled`/`notifySyncOffline`/`clearSyncAlert` for the watchdog's alert); guarded by `areNotificationsEnabled()`. **The two watchdog messages share one notification id**, so they replace rather than stack and one `clearSyncAlert` retires whichever is standing — they are two readings of the same condition and must never be on screen together. Every post is `setOnlyAlertOnce` — the channel is `IMPORTANCE_HIGH` and the setup-health check now re-runs on the ACL path (four broadcasts per journey), so without it a re-`notify` of an unchanged warning heads-up and sounds each time.
...
- `sync/SyncHealthTest.kt` — staleness truth table (below/at/above 6h; just signed in; signed in long ago and never synced; no sign-in stamp at all; a sign-in newer than the last success) + the watchdog decision (not stale; just signed in; never synced; stale with no prior force; the 6h boundary; inside the grace window; expired grace window; a force predating the last success; signed out while stale; signed out with a fresh sync). The offline cases are paired against their online twins on purpose: offline-and-stale still forces, offline-inside-the-grace-window still holds off, and only the alert past the grace window differs.
