...
- `sync/SyncHealth.kt` — pure domain: `SYNC_STALE_THRESHOLD_MILLIS` (6h) + `isSyncStale`, and the watchdog decision `syncRecoveryAction` (`NONE` / `FORCE_REENQUEUE` / `FORCE_REENQUEUE_AND_ALERT`) with its `FORCED_REENQUEUE_GRACE_MILLIS` (1h). Unit-tested.
...
- `sync/SyncScheduler.kt` — the only place WorkManager is enqueued: `ensureScheduled` (KEEP), `forceReenqueue` (CANCEL_AND_REENQUEUE — a genuinely new JobScheduler registration), `syncNow` (unique one-time manual run). The periodic request carries **no constraints**.
- `sync/SyncWatchdog.kt` — `recoverStalledSync(context)`: reads live sync state, applies `syncRecoveryAction`, forces a re-enqueue and posts the stall notification when a prior force has already had its grace window.
...
- `ui/MainActivity.kt` — single screen: sign-in, permission grant, manual sync (tagged `manual`, via `SyncScheduler.syncNow`), sign out, recent connect/disconnect list (last 10, heartbeats excluded), "Last alive check" line, "Last sync attempt" line (time + outcome); setup-health banner and sync-health banner (no successful sync in >6h) with one-tap fixes.
...
- `sync/SyncHealthTest.kt` — staleness truth table (below/at/above 6h) + the watchdog decision (not stale; stale with no prior force; the 6h boundary; inside the grace window; expired grace window; a force predating the last success).
...
- **JobScheduler can stop dispatching the periodic sync for weeks, with nothing wrong anywhere.** Twice now (Jul 2026, and 12 Aug – 1 Sep 2026 for 484h). App Standby Bucket `EXEMPTED`, Doze whitelisted, never force-stopped, WorkSpec `ENQUEUED` with a live JobScheduler job — the job simply was not run, and `ExistingPeriodicWorkPolicy.KEEP` is blind to present-but-not-dispatching, so the hundreds of `Application.onCreate` calls during the stall re-enqueued nothing. **We do not root-cause this; we mitigate and detect.** The ACL receiver is the watchdog: on each event, if no sync has succeeded in ≥6h it calls `SyncScheduler.forceReenqueue` (CANCEL_AND_REENQUEUE), waits out a 1h grace window, and if still stale forces again and posts a stall notification. Capture kept working perfectly throughout both stalls — one delivery mechanism dead, another perfect, same process.
