...
- `sync/SyncHealth.kt` — pure domain: `SYNC_STALE_THRESHOLD_MILLIS` (6h) + `isSyncStale`, and the watchdog decision `syncRecoveryAction` (`NONE` / `CLEAR_ALERT` / `FORCE_REENQUEUE` / `FORCE_REENQUEUE_AND_ALERT`) with its 1h forced-re-enqueue grace window. Signed-out is one of its arms, not a caller-side gate: with no account there is nothing to recover and the stall alert's own instruction is unreachable, so it is cleared. `isSyncStale` takes `lastAttemptMillis` as well as `lastSuccessMillis` and applies the never-attempted gate itself — an install whose worker has not yet run once is not stale, and no caller can forget that. Unit-tested.
...
- `sync/SyncState.kt` — `SharedPreferences` wrapper (account name, per-month offsets, last-attempt millis+outcome, last-success millis via `recordAttempt`, last forced re-enqueue via `recordForcedReenqueue`, whose only caller is `SyncScheduler.forceReenqueue`).
...
- `sync/SyncAttemptTest.kt` — JSON round-trip (all fields, null error, every outcome, quoted error), garbage/unknown-outcome rejection, trigger fallback, `isClean` set, `unattended` set.
- `sync/SyncJournalPolicyTest.kt` — retention under/at/over the cap.
- `sync/SyncHealthTest.kt` — staleness truth table (below/at/above 6h; never attempted; attempted but never succeeded) + the watchdog decision (not stale; never attempted; attempted but never succeeded; stale with no prior force; the 6h boundary; inside the grace window; expired grace window; a force predating the last success; signed out, stale and fresh).
